...
const CharacterQuote = ()=>{
    ...
    return (
        <>
            ...
            {
                quote ? (
                    <section className='mt-5 max-w-7xl mx-auto p-4 focus:outline-none sm:p-3 md:p-5'>
                        <div className="grid grid-cols-1 gap-6 lg:gap-8 sm:grid-col-2 lg:grid-cols-3 max-h-[33rem]">
                            {
                                quote.quotes.map((q, index) => (
                                    <figure key={index} className="cursor-events-auto relative flex flex-col-reverse bg-slate-50 rounded-lg p-6 ring-1 ring-slate-300 hover:ring-slate-400">
